...
Public help is available at `/faq` and `/contact`, without a login. The FAQ explains campaign applications, approvals, project access and completed-campaign summaries. The contact form emails Ran and Noam by default (override with server-only `GOODRAISE_CONTACT_EMAILS`) using the existing transactional-email service. Local development captures messages in the ignored email outbox; actual delivery requires the production provider configuration. See [contact delivery and safeguards](docs/development.md#public-contact-form).

Accounts cannot self-register. A site admin approves the email and assigns one or more organization/campaign memberships; the user's first login then enters password setup. `/admin` opens the assigned-project selector, with active and completed projects separated and a clear no-project state when there are none. Login can open the only assigned project directly for non-site-admins; explicit My Projects links always show the selector. Each card has a role-appropriate action. Campaign navigation lives inside the selected campaign with its name and a return link, not in the global header. Site admins manage approvals and memberships at `/admin/users`.
