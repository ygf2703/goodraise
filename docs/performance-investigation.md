...
| --- | --- | --- | --- |
| 2 | 6 | 2, each bounded to one record | 0 |
| 50 | 6 | 2, each bounded to one record | 0 |
...
The same permission cases run against development storage and PostgreSQL: role thresholds, cross-tenant access (including duplicate slugs), ID/slug assignments, incomplete legacy scope, route/query precedence, changed assignments, disabled managers and revoked sessions. The auth-status list now uses the same authorization policy as direct access, including organization-ID assignments. Existing campaign registry/portfolio response checks remain in the general regression suite.
...
- **Account writes:** lookup-time seeding processes only the requested account and uses `IS DISTINCT FROM` to skip unchanged updates. Current configured roles/assignments/active state are still checked on every request. The existing expired-session cleanup remains unchanged; moving it requires a defined maintenance schedule.
