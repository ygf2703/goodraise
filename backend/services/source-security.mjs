...
export class SourceConfigValidationError extends Error {}

export async function validateSourceConfig(config) {
  if (config?.mode !== "api" || !config.api?.endpoint) return;
  try {
    await validateExternalUrl(config.api.endpoint);
  } catch (error) {
    throw new SourceConfigValidationError(error instanceof Error ? error.message : "כתובת מקור הנתונים אינה תקינה.");
  }
}
