...
import { buildCampaignProjectDates } from "../backend/services/campaign-store.mjs";
...
test("preserves the snapshot analysis window until the configuration save flow updates it", () => {
  ...
  assert.equal(dataset.meta.defaultFrom, "2026-03-15");
  assert.equal(dataset.meta.defaultTo, "2026-03-15");
  assert.equal(dataset.meta.projectDates.length, 1);
});
