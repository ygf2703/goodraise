...
test("prize rankings exclude campaign ambassadors who do not participate in the competition", async () => {
  const template = await readFile(new URL("../apps/web/src/compat/dashboard-controller.js", import.meta.url), "utf8");
  assert.match(template, /state.prizeModel.excludedAmbassadors/);
  assert.doesNotMatch(template, /מועדון הכדורגל מכבי תל אביב|לזכרו של כליל קמחי/);
...
test("prize dashboard calculates a sprint winner from a precise selected time window", async () => {
  ...
  assert.match(layout, /id="time-from-filter"/);
  assert.match(layout, /id="time-to-filter"/);
